    ...
    <div className="space-y-6 animate-fade-in max-w-4xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 border border-[#e2dcd4] shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="hidden sm:inline">View Live Changes</span>
          </button>
      ...
      <div className={`bg-white p-6 sm:p-8 border ${isEditing ? 'border-[#c5a059] ring-1 ring-[#c5a059]/20' : 'border-[#e2dcd4]'} shadow-sm space-y-4 transition-colors`}>
        ...
        <form onSubmit={handleSaveHero} className="space-y-4">
          <fieldset disabled={!isEditing} className={`space-y-4 ${!isEditing ? 'opacity-80 grayscale-[20%]' : ''}`}>
          <div>
          ...
          {isEditing && (
            <div className="pt-2 flex justify-end gap-3 animate-fade-in">
              <button
                type="button"
                onClick={() => setIsEditing(false)}
                className="px-4 py-2 bg-stone-100 text-stone-700 hover:bg-stone-200 rounded text-xs font-bold uppercase tracking-wider transition-all cursor-pointer"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="px-6 py-2 bg-[#1e1b18] hover:bg-[#c5a059] text-white hover:text-[#1e1b18] rounded text-xs font-bold uppercase tracking-widest transition-all shadow cursor-pointer"
              >
                <Save className="w-4 h-4 inline mr-2 -mt-0.5" />
                Publish Changes
              </button>
            </div>
          )}
